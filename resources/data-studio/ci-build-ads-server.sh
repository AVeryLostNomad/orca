...
curl -sL -o /tmp/pgsql.vsix "https://github.com/microsoft/azuredatastudio-postgresql/releases/download/v$PGSQL_VERSION/$PGSQL_ASSET"
"$PYTHON_BIN" -c "import zipfile; zipfile.ZipFile('/tmp/pgsql.vsix').extractall('/tmp/pgsql-vsix')"
cp -R /tmp/pgsql-vsix/extension extensions/azuredatastudio-postgresql
echo "::endgroup::"
...
if [[ "$PLATFORM" == "win32" ]]; then
  curl -sL -o /tmp/node-dist.zip "https://nodejs.org/dist/v$ADS_NODE_VERSION/node-v$ADS_NODE_VERSION-$NODE_DIST.zip"
  "$PYTHON_BIN" -c "import zipfile; zipfile.ZipFile('/tmp/node-dist.zip').extractall('/tmp/node-dist')"
  cp "/tmp/node-dist/node-v$ADS_NODE_VERSION-$NODE_DIST/node.exe" "$NODE_RUNTIME_DIR/node.exe"
else
  curl -sL "https://nodejs.org/dist/v$ADS_NODE_VERSION/node-v$ADS_NODE_VERSION-$NODE_DIST.tar.gz" | tar -xz -C /tmp
  cp "/tmp/node-v$ADS_NODE_VERSION-$NODE_DIST/bin/node" "$NODE_RUNTIME_DIR/node"
