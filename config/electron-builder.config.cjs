...
module.exports = {
  ...
  // Fork: register each supported editor type. macOS uses Alternate so users'
  // existing default handlers remain untouched.
  ...
  win: {
    executableName: 'Orca',
    // Fork Windows installers are unsigned. Keep the upstream relay hook so a
    // future opt-in signing pipeline can embed a returned NSIS uninstaller, but
    // never claim SignPath's publisher identity or require it for updates.
    signtoolOptions: {
  ...
  nsis: {
    ...
    // electron-builder allows one include, so the relocated-daemon uninstall
    // sweep and the additive markdown Open With registration share this hook.
  ...
  publish: {
    provider: 'github',
    // Fork stable/RC releases update from AVeryLostNomad/orca. The upstream
    // dev-channel feeds remain their separate historical stablyai repositories.
    owner: devChannelRepo ? 'stablyai' : 'AVeryLostNomad',
