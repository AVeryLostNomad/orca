...
describe('electron-builder dev-channel identity', () => {
  it('keeps fork Windows builds unsigned without a SignPath publisher requirement', () => {
    ...
    expect(config.publish.owner).toBe('AVeryLostNomad')
  ...
  // The relay is inert unless a future CI pipeline supplies its paths. It must
  // not add a publisher assertion to any fork channel.
  it('carries the inert uninstaller relay without a publisher requirement', () => {
