...
const lifecycle = vi.hoisted(() => ({
  events: [] as string[],
  diffModifiedContents: [] as string[],
...
vi.mock('@/lib/lazy-with-retry', async () => {
  ...
  return {
    lazyWithRetry: (factory: () => Promise<unknown>) => {
      if (factory.toString().includes('PierreFileDiff')) {
        return function MockPierreFileDiff(props: {
          relativePath: string
          modifiedContent: string
        }) {
          lifecycle.diffModifiedContents.push(props.modifiedContent)
          /* oxlint-disable react-hooks/exhaustive-deps -- Mount-only by design: a prop-effect would hide a missing outer React remount. */
          React.useEffect(() => {
            lifecycle.events.push(`mount-diff:${props.relativePath}`)
            return () => {
              lifecycle.events.push(`unmount-diff:${props.relativePath}`)
            }
          }, [])
...
afterEach(() => {
  ...
  lifecycle.diffModifiedContents.length = 0
...
describe('EditorContent Monaco lifecycle boundary', () => {
  ...
  it('keeps the diff renderer mounted when a save updates the modified content', () => {
    // Why: saves must flow through props (content cacheKey identity) without
    // remounting the diff renderer and dropping its scroll position.
    ...
    expect(lifecycle.events).toEqual(['mount-diff:notes.ts'])
    expect(lifecycle.diffModifiedContents).toEqual(['first save', 'second save'])
  })

  it('still remounts the diff renderer for an explicit reload request', () => {
    const diff = file('/repo/notes.ts', { mode: 'diff', diffSource: 'unstaged' })
