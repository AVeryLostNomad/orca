...
export function getWorktreeJumpPaletteResultCount(
  ...
  if (controller.paletteMode === 'files') {
    return controller.fileItems.length + Number(controller.showQuickAi)
  }
  return controller.hasQuery
    ? controller.worktreeItems.length +
        ...
        controller.fileItems.length +
        Number(controller.showQuickAi)
