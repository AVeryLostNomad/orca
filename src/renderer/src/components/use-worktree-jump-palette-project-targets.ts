...
import { buildAutomationCommandItems } from '@/components/cmd-j/automation-command-items'
...
export function useWorktreeJumpPaletteProjectTargets({
  ...
  const activeWorktreeRepoId = useMemo(
    () => allWorktrees.find((worktree) => worktree.id === activeWorktreeId)?.repoId ?? null,
  ...
  const actionResults = useMemo(
    ...
    [activeWorktreeRepoId, commandBarAutomations, pluginCommands, settings?.terminalQuickCommands]
  )
