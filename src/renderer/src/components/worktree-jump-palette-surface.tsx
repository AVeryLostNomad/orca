...
export function WorktreeJumpPaletteSurface({
  ...
  const hasPaletteResults =
    ...
    controller.fileItems.length > 0 ||
    controller.showQuickAi
  const paletteDialog = (
    <CommandDialog
    ...
    >
      <CommandInput
        ...
        onKeyDown={controller.handlePaletteInputKeyDown}
      ...
      >
        {controller.paletteMode === 'files' && controller.filesLoadError ? (
          <FileModeLoadError error={controller.filesLoadError} />
        ) : controller.paletteMode === 'files' && controller.filesLoading && !hasPaletteResults ? (
          <div className="py-6 text-center text-sm text-muted-foreground">
            {translate('auto.components.WorktreeJumpPalette.loadingFiles', 'Loading files...')}
          </div>
        ) : controller.isLoading && !hasPaletteResults && !controller.showCreateAction ? (
          <PaletteState
            title={translate(
          ...
          />
        ) : !hasPaletteResults && !controller.showCreateAction ? (
...
function FileModeLoadError({ error }: { error: string }): React.JSX.Element {
  const guidance = parseQuickOpenInstallRgGuidance(error)
  return guidance ? (
    <QuickOpenInstallRgGuidance
