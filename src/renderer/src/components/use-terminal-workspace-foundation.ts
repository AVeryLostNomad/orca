...
import { buildProjectGroupChildIndex } from '../../../shared/project-groups'
...
export function useTerminalWorkspaceFoundation() {
  ...
  const persistedFolderWorkspaces = useAppStore((state) => state.folderWorkspaces)
  const projectGroups = useAppStore((state) => state.projectGroups)
  const repos = useAppStore((state) => state.repos)
  const folderWorkspaces = useMemo(() => {
    if (projectGroups.length === 0) {
      return persistedFolderWorkspaces
    }
    const workspaces = [...persistedFolderWorkspaces]
    const childGroupIndex = buildProjectGroupChildIndex(projectGroups)
    for (const group of projectGroups) {
      const workspace = projectGroupToFolderWorkspace({
        group,
        projectGroups,
        repos,
        childGroupIndex
    ...
    return workspaces
  }, [persistedFolderWorkspaces, projectGroups, repos])
