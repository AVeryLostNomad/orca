...
export type VisibleTabRef = {
  type: 'terminal' | 'editor' | 'agent-session' | 'browser' | 'simulator' | 'vscode' | 'datastudio'
  id: string
  tabId?: string
}
...
export function getGroupVisibleTabOrder(
  ...
  const toRef = (tab: Tab): VisibleTabRef | null => {
    ...
    if (tab.contentType === 'vscode') {
      return codeServerEntityIds.has(tab.entityId)
        ? { type: 'vscode', id: tab.entityId, tabId: tab.id }
        : null
    }
    if (tab.contentType === 'datastudio') {
      return dataStudioEntityIds.has(tab.entityId)
        ? { type: 'datastudio', id: tab.entityId, tabId: tab.id }
        : null
    }
  ...
  const codeServerIds: string[] = []
  const dataStudioIds: string[] = []
  const idsByType = {
    ...
    vscode: codeServerIds,
    datastudio: dataStudioIds
  }
...
export function getActiveTabNavOrder(
  ...
  const simulatorIds =
    ...
    (state.unifiedTabsByWorktree[worktreeId] ?? [])
      ...
      .map((tab) => tab.id)
  const codeServerIds =
    ids.codeServerIds ?? (state.codeServerTabsByWorktree[worktreeId] ?? []).map((tab) => tab.id)
  const dataStudioIds =
    ids.dataStudioIds ?? (state.dataStudioTabsByWorktree[worktreeId] ?? []).map((tab) => tab.id)
