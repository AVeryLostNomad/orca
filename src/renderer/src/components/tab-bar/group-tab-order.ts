...
export function getGroupVisibleTabOrder(
  ...
  for (const unifiedId of group.tabOrder) {
    ...
    } else if (tab.contentType === 'vscode') {
      // Why: vscode mirrors browser (id = backing entity, tabId = unified id) so
      // MRU/cycle activation can target the code-server tab, not the editor.
      if (!codeServerEntityIds.has(tab.entityId) || seenCodeServers.has(tab.entityId)) {
        continue
      }
...
export function getActiveTabNavOrder(
  ...
  const simulatorIds =
    ...
    (state.unifiedTabsByWorktree[worktreeId] ?? [])
      ...
      .map((tab) => tab.id)
  // Why: vscode keys by backing entity id (like browser), so derive from the
  // unified tabs' entityId to match the getGroupVisibleTabOrder membership check.
  const codeServerIds =
    ids.codeServerIds ??
    (state.unifiedTabsByWorktree[worktreeId] ?? [])
