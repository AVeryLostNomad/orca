...
import { Database, X, Pin, PinOff } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
// Why: mirror CodeServerTab's chip shell (sortable scaffolding, active
// indicator, width/close-button classes, context menu) so datastudio chips
// reorder, highlight, and close identically — only the glyph differs.
export default function DataStudioTab({
  label,
  isActive,
  isPinned,
  hasTabsToRight,
  ...
  // Why: the workbench mounts as an overlay outside the renderer document, so
  // outside-click detection misses it. Dismiss on window blur.
  useEffect(() => {
    if (!menuOpen) {
      return
    }
  ...
  const tabRoot = (
    ...
    >
      ...
      <Database className="size-3 mr-1 shrink-0 text-amber-500" />
      ...
      <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen} modal={false}>
        ...
        >
          <DropdownMenuItem onSelect={onTogglePin}>
            ...
            {isPinned
              ? translate('auto.components.tab.bar.DataStudioTab.1f7666419b', 'Unpin Tab')
              : translate('auto.components.tab.bar.DataStudioTab.52fa615dce', 'Pin Tab')}
          </DropdownMenuItem>
          ...
          <DropdownMenuItem onSelect={() => !isPinned && onClose()} disabled={isPinned}>
            {translate('auto.components.tab.bar.DataStudioTab.c4dee16431', 'Close')}
          </DropdownMenuItem>
          <DropdownMenuItem onSelect={onCloseToRight} disabled={!hasTabsToRight}>
            {translate(
              'auto.components.tab.bar.DataStudioTab.31dc5c0d4f',
