...
export class OrcaRuntimeWithTransitionGraphReloadToTerminalState extends OrcaRuntimeWithHasTerminalsForWorktree {
  ...
  protected findFolderWorkspaceById(folderWorkspaceId: string): FolderWorkspace | null {
    return (
      this.store?.getFolderWorkspaces?.().find((entry) => entry.id === folderWorkspaceId) ??
      findProjectGroupWorkspaceFolder(
        folderWorkspaceId,
  ...
  protected resolveFolderWorkspaceSelector(selector: string): FolderWorkspace | null {
    ...
    const workspace = this.findFolderWorkspaceById(parsed.folderWorkspaceId)
    if (!workspace) {
