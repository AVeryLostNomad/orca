...
export class RuntimeRepositoryRegistrationController {
  ...
  async create(
    ...
    if (repoKind === 'git') {
      const error = await this.initializeGitRepo(
        targetPath,
        createdDir,
  ...
  private async initializeGitRepo(
    ...
    try {
      ...
      step = 'commit'
      if (authorIdentity) {
        await gitExecFileAsync(['config', 'user.name', authorIdentity.name], { cwd: targetPath })
        await gitExecFileAsync(['config', 'user.email', authorIdentity.email], { cwd: targetPath })
      }
      ...
      if (step === 'commit' && /Please tell me who you are|user\.name|user\.email/i.test(message)) {
        return 'Git author identity is not configured. Connect a GitHub account in Orca or configure Git author identity on this host, then try again.'
      }
      const label =
