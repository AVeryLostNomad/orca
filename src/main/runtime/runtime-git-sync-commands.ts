...
export class RuntimeGitSyncCommands {
  ...
  async moveRuntimeGitChanges(
    ...
    // A stash transfer relies on one shared object database, so never route either side
    // through a different execution host.
    ...
    const provider = requireRuntimeGitProvider(source)
    if (provider) {
