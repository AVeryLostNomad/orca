...
export function getPiAgentStatusHandlerSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    ...captureSessionMetadata,
    '    // Why: current OMP marks continuation settles explicitly; its final settle',
    '    // can arrive while ctx.isIdle() is still false until this handler returns.',
    "    if (isOmpRuntime() && event && typeof event === 'object' && 'willContinue' in event) {",
    '      clearPendingAgentEndCheck()',
    '      if (!(event as { willContinue?: boolean }).willContinue) postAgentEndOnce()',
    '      return',
    '    }',
