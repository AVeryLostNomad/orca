...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '// Why: in-process OMP subagents share this module; their lifecycle must never displace the',
    "// lead's pending status, so they get a lower-priority slot of their own.",
    'let pendingSubagentPost: PendingPost | null = null',
    ...
    '',
    'function post(hookEventName: string, extra: Record<string, unknown> = {}, options: { subagent?: boolean } = {}): void {',
    '  const ompRuntime = isOmpRuntime()',
    '  const next: PendingPost = {',
    '    hookEventName,',
    '    extra,',
    "    // Why: a child's session id is not the pane's resume id.",
    '    metadata: options.subagent ? {} : getPostSessionMetadata(ompRuntime),',
    '    ompRuntime,',
    '  }',
    '  if (options.subagent) pendingSubagentPost = next',
    '  else pendingPost = next',
    '  drainPosts()',
    '}',
    '',
    'function drainPosts(): void {',
    '  if (activePost) return',
    '  const next = pendingPost ?? pendingSubagentPost',
    '  if (!next) return',
    '  if (next === pendingPost) pendingPost = null',
    '  else pendingSubagentPost = null',
    '  activePost = true',
