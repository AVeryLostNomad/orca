...
function requestDesktopActivation(argv: readonly string[] = []): void {
  ...
  state.openWithFiles.captureFromArgv(
    argv.filter((argument) => markdownPathsFromArguments([argument]).length === 0),
    publishOpenWithFiles
  )
...
if (preflightReady) {
  ...
  // Why: macOS delivers "Open With" before ready and only to a handler that claims the event.
  // Keep the upstream markdown-document lane and the fork's editor-file lane live together.
  app.on('open-file', (event, filePath) => {
    const openedMarkdown = state.osOpenedMarkdownFiles.captureFilePaths(
      [filePath],
      publishOsOpenedMarkdownFiles
    )
    const openedWith = openedMarkdown
      ? false
      : state.openWithFiles.capture([filePath], publishOpenWithFiles)
    if (!openedMarkdown && !openedWith) {
      return
    }
    event.preventDefault()
    if (openedWith) {
  ...
  // Why no publish: nothing is listening this early, so the first renderer pulls these on mount.
  state.openWithFiles.captureFromArgv(
    process.argv.filter((argument) => markdownPathsFromArguments([argument]).length === 0)
  )
