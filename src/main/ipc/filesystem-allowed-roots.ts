...
import { deriveProjectGroupWorkspacePath } from '../../shared/project-group-workspace'
...
function getLocalFolderScopeRoots(store: Store, repos: readonly Repo[]): string[] {
  ...
  for (const group of projectGroups) {
    if (group.connectionId || (group.executionHostId && group.executionHostId !== 'local')) {
      continue
    }
    const groupPath = deriveProjectGroupWorkspacePath({
      group,
      projectGroups,
      repos,
      childGroupIndex
