...
import { buildCodeServerArgs } from './code-server-manager'
...
export type CodeServerProfile = {
  ...
  buildSpawn(port: number): { command: string; args: string[]; env?: Record<string, string> }
...
export function createEditorProfile(): CodeServerProfile {
  const userDataDir = getCodeServerUserDataDir()
  const extensionsDir = getCodeServerExtensionsDir()
  return {
    key: 'editor',
    userDataDir,
    extensionsDir,
    ...
    resolveInstalled: resolveCodeServerExecutable,
    ...
    buildSpawn(port) {
      ...
      return { command: exe, args: buildCodeServerArgs(port, { userDataDir, extensionsDir }) }
    },
