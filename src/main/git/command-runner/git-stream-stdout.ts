...
import {
  applyGitAccountEnv,
  invalidateGitAccountTokenForAuthFailure,
  isGitAuthFailure,
  resolveGitAccountForCommand
...
export async function gitStreamStdout(
  ...
  return withGitSpan({ args, cwd: options.cwd }, async (span) => {
    ...
    const account = await resolveGitAccountForCommand(args, options.cwd, gitOptions.env)
    const accountEnv = applyGitAccountEnv(nonInteractiveGitEnv(gitOptions.env), account)
    ...
    try {
      try {
        ...
        if (isGitAuthFailure(error)) {
          invalidateGitAccountTokenForAuthFailure(account)
        }
        throw error
      }
    } finally {
