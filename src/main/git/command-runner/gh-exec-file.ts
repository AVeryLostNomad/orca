...
import {
  applyGhAccountEnv,
  ghAccountRateLimitScope,
  refreshGhAccountAfterAuthFailure,
  resolveGhAccountForCommand
...
export type GhExecOptions = Omit<GitExecOptions, 'cwd'> & {
  ...
  host?: string
  /** Repo path used only for pinned-account lookup when gh runs cwd-less for SSH. */
  accountCwdHint?: string
  /** Bypass selected-account injection for gh auth and credential probes. */
...
export async function ghExecFileAsync(
  ...
  let account = await resolveGhAccountForCommand(args, options)
  assertGhRateLimitScopeAvailable(
    ...
    rateLimitBucket,
    rateLimitProbe,
    ghAccountRateLimitScope(account)
  )
    ...
    try {
      ...
      const { stdout, stderr } = await execFileCaptureToTermination(
        ...
        resolved.args,
        {
          ...
          timeout: timeoutMs,
          env: applyGhAccountEnv(nonInteractiveGhEnv(options.env), account),
      ...
      if (isGhPrimaryRateLimitStderr(stderr)) {
        notifyGhPrimaryRateLimit(
          rateLimitBucket,
          ghRateLimitScope(args, options, resolved, ghAccountRateLimitScope(account))
        )
      }
      if (
        account &&
        ...
        const refreshed = await refreshGhAccountAfterAuthFailure(account, args, options)
        if (refreshed && refreshed.token !== account.token) {
          account = refreshed
          attempt = -1
          continue
        }
