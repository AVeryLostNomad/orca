...
describe('SshGitProvider', () => {
  ...
  it('moveChangesToWorktree sends the narrow cross-worktree request', async () => {
    mux.request.mockResolvedValue({ status: 'moved' })

    await expect(
      provider.moveChangesToWorktree('/home/user/source', '/home/user/target')
    ).resolves.toEqual({ status: 'moved' })
    expect(mux.request).toHaveBeenCalledWith('git.moveChanges', {
      worktreePath: '/home/user/source',
      targetWorktreePath: '/home/user/target'
    })
  })

  it('moveChangesToWorktree maps old relays to the reconnect message', async () => {
    mux.request.mockRejectedValueOnce(methodNotFound('git.moveChanges'))
