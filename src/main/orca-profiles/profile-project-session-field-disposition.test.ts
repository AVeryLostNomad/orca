...
describe('client-hosted rows in the repo-removal and transfer paths', () => {
  ...
  it('removes and rekeys embedded editor tabs with their owning worktree', () => {
    const session: WorkspaceSessionState = {
      ...getDefaultWorkspaceSession(),
      codeServerTabsByWorktree: {
        [REMOVED_WORKTREE_ID]: [
          { id: 'vscode-1', worktreeId: REMOVED_WORKTREE_ID, folderPath: '/a', label: 'VS Code' }
        ],
        [RETAINED_WORKTREE_ID]: [
          { id: 'vscode-2', worktreeId: RETAINED_WORKTREE_ID, folderPath: '/b', label: 'VS Code' }
        ]
      },
    ...
    expect(
      removeRepoFromWorkspaceSession(session, REMOVED_REPO_ID).codeServerTabsByWorktree
    ).toEqual({
      [RETAINED_WORKTREE_ID]: [
        { id: 'vscode-2', worktreeId: RETAINED_WORKTREE_ID, folderPath: '/b', label: 'VS Code' }
      ]
    })
    const transferred = extractSessionForTransfer(session, REMOVED_REPO_ID, TRANSFER_TARGET_REPO_ID)
    expect(transferred.codeServerTabsByWorktree).toEqual({
      'repo-3::/tmp/worktree-a': [
        {
          id: 'vscode-1',
          worktreeId: 'repo-3::/tmp/worktree-a',
          folderPath: '/a',
          label: 'VS Code'
        }
      ]
    })
    expect(transferred.dataStudioTabsByWorktree).toEqual({
      'repo-3::/tmp/worktree-a': [
        {
          id: 'ads-1',
          worktreeId: 'repo-3::/tmp/worktree-a',
          repoId: TRANSFER_TARGET_REPO_ID,
          folderPath: '/a',
          label: 'Data Studio'
        }
      ]
