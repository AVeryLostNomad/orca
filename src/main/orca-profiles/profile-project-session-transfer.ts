...
function hasTransferredSessionState(session: WorkspaceSessionState): boolean {
  return (
    ...
    Object.keys(session.codeServerTabsByWorktree ?? {}).length > 0 ||
    Object.keys(session.dataStudioTabsByWorktree ?? {}).length > 0 ||
...
export function extractSessionForTransfer(
  ...
  transferred.codeServerTabsByWorktree = mapOwnerRecord(source.codeServerTabsByWorktree, (tabs) =>
    tabs.map((tab) => rekeyCodeServerTab(tab, oldRepoId, newRepoId))
  )
  transferred.dataStudioTabsByWorktree = mapOwnerRecord(source.dataStudioTabsByWorktree, (tabs) =>
    tabs.map((tab) => rekeyDataStudioTab(tab, oldRepoId, newRepoId))
  )
...
function rekeyDataStudioTab(
  ...
  return {
    ...structuredClone(tab),
    worktreeId: rekeyWorktreeId(oldRepoId, newRepoId, tab.worktreeId),
    repoId: tab.repoId === oldRepoId ? newRepoId : tab.repoId
  }
}
