...
describe('updater', () => {
  ...
  it('retries a prerelease check once against the previous feed tag when the manifest is missing', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenCalledWith({
        provider: 'generic',
        url: 'https://github.com/AVeryLostNomad/orca/releases/download/v1.3.51-rc.7'
      })
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/AVeryLostNomad/orca/releases/download/v1.3.51-rc.6'
      })
    })
  ...
  it('pins the generic feed to a concrete stable tag for a stable user', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
      provider: 'generic',
      url: 'https://github.com/AVeryLostNomad/orca/releases/download/v1.3.18'
    })
  })
